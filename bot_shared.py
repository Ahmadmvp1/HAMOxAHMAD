...
USERNAME = "ddama9026@gmail.com"
PASSWORD = "LkPePo77@"
BOT_TOKEN = "8347417707:AAEq4Lv86NPdHkX5B3GWXlXwvFSUHyYg1dc"
CHAT_IDS = [
    "-1003837925104",   # الجروب
    "-1003843893866",      # القناة
]
REFRESH_INTERVAL = 3
TIMEOUT = 100
MAX_RETRIES = 5
...
ADMIN_IDS = [8280587120, 6634032038]
DB_PATH = "bot.db"
...
def send_welcome(message):
    ...
    if private_combo and private_combo in COUNTRY_CODES:
        ...
        buttons.append(types.InlineKeyboardButton(f"{flag} {name} (Private)", callback_data=f"country_{private_combo}_1", style="primary"))
    ...
    if is_admin(user_id):
        markup.add(types.InlineKeyboardButton("🔐 Admin Panel", callback_data="admin_panel", style="primary"))

    # 6. الرسالة الترحيبية المنسقة
    fancy_text = (
        "<b>❍<u>hamo&AHMED • 𝑩𝒐𝒕 </u>❍</b>\n\n"
        "<b>🔋 <u>مـنـور بـوت • hamo&AHMED • يـحـب</u></b>\n\n"
        "<b>🎓 <u>الـمـطـورون</u>  • <a href='https://t.me/A7mad_1m'>Ahmad</a> & <a href='https://t.me/MO7_TF7'>Hamo</a></b>\n\n"
        "<b>────────────────────</b>\n"
...
def handle_country_selection(call):
    ...
    if not available_numbers:
        ...
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("🔙 العودة لاختيار دولة أخرى", callback_data="back_to_countries", style="primary"))
    ...
    # زر الجروب في الأعلى
    markup.add(types.InlineKeyboardButton("OPT GROUP", url="https://t.me/+g5zHkO0C0tAzYzU1", style="success"))
    
    # أزرار التحكم في صف واحد
    markup.row(
        types.InlineKeyboardButton("🔄 Change Number", callback_data=f"change_num_{country_code}_{combo_index}", style="primary"),
        types.InlineKeyboardButton("🔙 Back", callback_data="back_to_countries", style="primary")
    )
...
def back_to_countries(call):
    ...
    if is_admin(call.from_user.id):
        admin_btn = types.InlineKeyboardButton("🔐 Admin Panel", callback_data="admin_panel", style="primary")
        markup.add(admin_btn)

    # 2. النص المنسق (نفس تنسيق رسالة الترحيب)
    fancy_text = (
        "<b>❍<u>hamo&AHMED • 𝑩𝒐𝒕𝒕 </u>❍</b>\n\n"
        "<b>🔋 <u>مـنـور بـوت • hamo&AHMED • يـحـب</u></b>\n\n"
        "<b>🎓 <u>الـمـطـورون</u>  • <a href='https://t.me/A7mad_1m'>Ahmad</a> & <a href='https://t.me/MO7_TF7'>Hamo</a></b>\n\n"
        "<b>────────────────────</b>\n"
        "<b><u>اخـتـر الــدولـة الـتـي تـريـدهـا مـن الـزر الاسـفـل</u> ⬇️</b>"
...
def admin_main_menu():
    ...
    markup.add(types.InlineKeyboardButton(f"{status_icon} {status_text} {status_icon}", callback_data="toggle_maintenance", style="primary"))
    ...
    markup.row(
        ...
        types.InlineKeyboardButton("📨 إذاعة مخصصة", callback_data="admin_broadcast_user", style="primary")
    )
    
    # 5. قسم إدارة المستخدمين
    markup.row(
        types.InlineKeyboardButton("🚫 حظر", callback_data="admin_ban", style="danger"),
        types.InlineKeyboardButton("✅ إلغاء حظر", callback_data="admin_unban", style="danger"),
        types.InlineKeyboardButton("👤 معلومات", callback_data="admin_user_info", style="primary")
    )
    
    # 6. قسم الإعدادات المتقدمة
    markup.row(
        types.InlineKeyboardButton("🔗 إشتراك", callback_data="admin_force_sub", style="primary"),
        types.InlineKeyboardButton("🖥️ اللوحات", callback_data="admin_dashboards", style="primary"),
        types.InlineKeyboardButton("🔑 برايفت", callback_data="admin_private_combo", style="primary")
    )

    # 7. زر الخروج
    markup.add(types.InlineKeyboardButton("🔙 مغادرة لوحة التحكم", callback_data="back_to_countries", style="primary"))
...
def admin_force_sub(call):
    ...
    markup.add(types.InlineKeyboardButton("➕ إضافة قناة", callback_data="add_force_ch", style="primary"))
    markup.add(types.InlineKeyboardButton("🔙 Back", callback_data="admin_panel", style="primary"))
...
def add_force_ch_step1(call):
    ...
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("🔙 Back", callback_data="admin_force_sub", style="primary"))
...
def edit_force_ch(call):
    ...
    if enabled:
        markup.add(types.InlineKeyboardButton("❌ تعطيل", callback_data=f"toggle_ch_{ch_id}", style="primary"))
    else:
        markup.add(types.InlineKeyboardButton("✅ تفعيل", callback_data=f"toggle_ch_{ch_id}", style="success"))
    markup.add(types.InlineKeyboardButton("🗑️ حذف", callback_data=f"del_ch_{ch_id}", style="primary"))
    markup.add(types.InlineKeyboardButton("🔙 Back", callback_data="admin_force_sub", style="primary"))
    bot.edit_message_text(text, call.message.chat.id, call.message.message_id, reply_markup=markup)
...
def edit_desc_step1(call):
    ...
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("🔙 Back", callback_data=f"edit_force_ch_{ch_id}", style="primary"))
        ...
        if country_code in COUNTRY_CODES:
            ...
            for idx in indices:
                ...
                if len(indices) == 1:
                    ...
                    btn_text = f"{flag} {name} ({idx})"
                markup.add(types.InlineKeyboardButton(btn_text, callback_data=f"del_combo_{country_code}_{idx}", style="danger"))
...
def admin_private_combo(call):
    if not is_admin(call.from_user.id):
        return
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("➕ إضافة كومبو برايفت", callback_data="add_private_combo", style="primary"))
    markup.add(types.InlineKeyboardButton("🗑️ مسح كومبو برايفت", callback_data="del_private_combo", style="primary"))
    markup.add(types.InlineKeyboardButton("🔙 Back", callback_data="admin_panel", style="primary"))
...
def add_private_combo_step1(call):
    ...
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("🔙 Back", callback_data="admin_private_combo", style="primary"))
    bot.edit_message_text("أدخل معرف المستخدم:", call.message.chat.id, call.message.message_id, reply_markup=markup)
...
def send_otp_to_user_and_group(date_str, number, sms):
    ...
    if user_id:
        try:
            markup = types.InlineKeyboardMarkup()
            markup.row(
                types.InlineKeyboardButton("𝐴ℎ𝑚𝑎𝑑⚙️", url="https://t.me/A7mad_1m", style="primary"),
                types.InlineKeyboardButton("𝐾: 𝑆𝑀𝑆", url="https://t.me/-1003767104268", style="primary")
            )
            bot.send_message(
                user_id,
    ...
    try:
        ...
        keyboard = {
            "inline_keyboard": [
                ...
                [
                    {"text": "💬 𝑆𝑀𝑆 ↗️", "url": "https://t.me/+g5zHkO0C0tAzYzU1", "style": "primary"},
                    {"text": "𝐵𝑂𝑇 ↗️", "url": "https://t.me/FREE_NUMBER7_OTP_BOT", "style": "success"}
                ],
                
                # الصف الثالث: زر المطورين
                [
                    {"text": "M:𝐵𝑂𝑇1 ↗️", "url": "https://t.me/A7mad_1m", "style": "danger"},
                    {"text": "M:𝐵𝑂𝑇2 ↗️", "url": "https://t.me/MO7_TF7", "style": "danger"}
                ]
            ]
        }
    except Exception as e:
